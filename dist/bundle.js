...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const calendar = new _fullcalendar_core__WEBPACK_IMPORTED_MODULE_0__.Calendar(calendarEl, {
    ...
    dayRender: function (info) {
      ...
      button.addEventListener('click', function () {
        openModal(info.dateStr); // Otwiera modal z datą
      });
  ...
  // Funkcja do aktualizacji dodatkowych opcji w formularzu
  window.updateOptions = function () {
    const taskSelect = document.getElementById('task');
    const additionalOptions = document.getElementById('additionalOptions');
    additionalOptions.innerHTML = ''; // Wyczyść wcześniejsze opcje

    if (taskSelect.value === 'sprawdzanieRaportów') {
      additionalOptions.innerHTML = `
                <label>Wybierz raport:</label>
                <select id="reportType" required>
                    <option value="1">Raport 1</option>
                    <option value="2">Raport 2</option>
                    <option value="3">Raport 3</option>
                </select>`;
    } else if (taskSelect.value === 'urlop') {
      additionalOptions.innerHTML = `
                <label>Typ urlopu:</label>
                <select id="leaveType" required>
                    <option value="platny">Płatny</option>
  ...
  document.getElementById('eventForm').addEventListener('submit', function (event) {
    ...
    // Zapisz dane do bazy danych (przykład, musisz dostosować do swojego backendu)
    const data = {
      employee: employee,
      task: task,
      date: document.getElementById('modalDate').textContent.split(': ')[1],
      additional: task === 'sprawdzanieRaportów' ? document.getElementById('reportType').value : document.getElementById('leaveType').value
    };

    // Wysyłka do backendu (przykład)
    fetch('/api/workday', {
      method: 'POST',
      headers: {
      ...
      closeModal(); // Zamknij modal po udanym zapisaniu
    }).catch(error => {
